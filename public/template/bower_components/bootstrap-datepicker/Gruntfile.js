module.exports = function (grunt) {
      ...
      js: {
        ...
        options: {
          replacements: [
            {
              pattern:
                /\$(\.fn\.datepicker\.version)\s=\s*("|\')[0-9\.a-z].*("|');/gi,
              replacement:
                "$.fn.datepicker.version = '" + grunt.option("newver") + "';",
            },
          ],
        },
      },
  ...
  // Code check tasks.
  grunt.registerTask("lint-js", "Lint all js files with jshint and jscs", [
    "jshint",
    "jscs",
  ]);
  grunt.registerTask("lint-css", "Lint all css files", [
    "dist-css",
    "csslint:dist",
  ]);
  grunt.registerTask("qunit-all", "Run qunit tests", [
    "qunit:main",
    "qunit-timezone",
  ]);
